  ...
  var require_src = __commonJS({
    "src/index.ts"(exports, module) {
      ...
      function prefetchSlide(slide, pluginConfig) {
        const canvases = slide.querySelectorAll("[data-handwriting]");
      ...
      var HandwritingReveal = {
        ...
        init(deck) {
          ...
          deck.on("slidechanged", (event) => {
            ...
            const canvases = currentSlide.querySelectorAll("[data-handwriting]");
            canvases.forEach((canvas) => {
            ...
            if (previousSlide) {
              const prev = previousSlide.querySelectorAll("[data-handwriting]");
              prev.forEach((canvas) => {
          ...
          deck.on("fragmentshown", (event) => {
            ...
            if (fragment instanceof HTMLCanvasElement && fragment.dataset.handwriting !== void 0) {
              animateCanvas(fragment, config);
            } else {
              const canvases = fragment.querySelectorAll("[data-handwriting]");
              canvases.forEach((canvas) => animateCanvas(canvas, config));
            }
          });
          deck.on("fragmenthidden", (event) => {
            const fragment = event.fragment;
            const canvases = fragment instanceof HTMLCanvasElement && fragment.dataset.handwriting !== void 0 ? [fragment] : Array.from(fragment.querySelectorAll("[data-handwriting]"));
            canvases.forEach((canvas) => {
              canvas.width = canvas.width;
            });
          });
          ...
          if (initialSlide) {
            ...
            const canvases = initialSlide.querySelectorAll("[data-handwriting]");
            canvases.forEach((canvas) => {
