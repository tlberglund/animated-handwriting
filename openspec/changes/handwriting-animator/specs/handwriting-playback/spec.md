...
The playback engine SHALL expose a class-based API that accepts an HTML canvas element and an exported glyph set JSON object. It SHALL provide a `write(text, options)` method that animates the given string on the canvas. The `options` parameter SHALL accept at minimum a `speed` multiplier (default 1.5) and a `color` value.
